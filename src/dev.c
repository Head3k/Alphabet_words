...
{              // function for decisions move for two words
    ...
    if (x == strlen(k1))
        return 0; // condition for move two words
    if (x == strlen(k2))
        return 1; // condition for move two words
    if (tolower(k1[x]) > tolower(k2[x]))
        return 1; // condition for move two words
    return 0;     // return NULL
}

char computing(char* array, int count_words, int count_symbols)
{ // function for symbols -> words and move words in array
...
{                          // function for enter data and check quantity symbols
